...
Both packages are publicly available and can be installed from Bioconductor version 2.8 or higher in R version 2.13.0 or higher. To locate the packages:
...
GTMLPreds<-MM2S_Mouse(InputMatrix=GTML,xls_output=TRUE,parallelize=1)
@
...
HumanPreds<-MM2S_Human(InputMatrix=HumanExpr,xls_output=TRUE,parallelize=1)
@
...
# We can view the first 20 sample replicates. 
